 ...
 *
 * Output is written in place next to the source. Generated paths are listed in
 * .gitignore, so the repo stays clean and Vercel regenerates on every deploy.
 */
...
const GI_START = "# >>> generated by scripts/build.js — do not edit this block";
const GI_END = "# <<< end generated block";

function updateGitignore() {
  const file = path.join(ROOT, ".gitignore");
  let current = fs.existsSync(file) ? fs.readFileSync(file, "utf8") : "";

  const entries = ["/index.html", "/sitemap.xml", "/robots.txt", "/help/"]
    .concat(Object.keys(DATA.guides).map((id) => "/" + id + "/"))
    .sort();

  const block = [
    GI_START,
    "# Sources are src/app.html, guides.json and safeguarding.json.",
    "# Vercel regenerates these on every deploy, so a committed copy only goes stale.",
    ...entries,
    GI_END
  ].join("\n");

  const startIdx = current.indexOf(GI_START);
  const endIdx = current.indexOf(GI_END);
  if (startIdx !== -1 && endIdx !== -1) {
    current = current.slice(0, startIdx) + block + current.slice(endIdx + GI_END.length);
  } else {
...
function build() {
  ...
  // Keep .gitignore in step with what we just generated, so adding a guide to
  // guides.json never leaves a stray build artifact in the repo.
  updateGitignore();
